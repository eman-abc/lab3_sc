...
public class RulesOf6005 {
     ...
     *        yourself or, in the case of a team project, your,
     *        otherwise false.
     ...
     */
	public static boolean mayUseCodeInAssignment(
	    ...
	    if (writtenByYourself) {
	        return true; // You can always use your own code
	    } 
	    if (availableToOthers && citingYourSource) {
	        return true; // You can use code that is available publicly and cited
	    } 
	    if (!writtenByYourself && availableToOthers && !citingYourSource) {
	        return false; // You cannot use publicly available code if it is not cited
	    }
	    if (!writtenAsCourseWork && !implementationRequired) {
	        return true; // You can use code that isn’t for coursework and doesn’t require specific implementation
	    }
